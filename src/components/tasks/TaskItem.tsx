...
import { Trash2, Check, Clock, Loader2, MessageSquare } from "lucide-react";
  ...
  return (
    ...
    >
      ...
      <button
        ...
        className="flex h-7 w-7 flex-shrink-0 items-center justify-center rounded-full border-2 transition-all hover:scale-110"
        style={{
          borderColor: activity ? activity.color : isOverdue ? "#f43f5e" : cfg.iconColor,
          backgroundColor:
            status === "done"
              ? cfg.iconColor
              : status === "in_progress"
      ...
      >
        {status === "done" ? (
          ...
          <Loader2 className={iconClass} style={{ color: cfg.iconColor }} />
        ) : (
          <Clock className="h-3.5 w-3.5" style={{ color: isOverdue ? "#f43f5e" : cfg.iconColor }} />
        )}
      </button>
      <span
        ...
        <div className="flex flex-wrap items-center gap-2">
          ...
          {showTimer && displaySeconds > 0 && (
            <span
              ...
              style={{
                backgroundColor: status === "in_progress" ? "rgba(251,191,36,0.15)" : "rgba(128,128,128,0.1)",
      ...
      <button
        ...
        style={{ opacity: hovered ? 1 : 0, color: fgMuted }}
        onMouseEnter={(e) => {
